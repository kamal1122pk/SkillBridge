...
async function fetchFreelancers(page = 1, search = "") {
  ...
  try {
    ...
    if (grid) grid.innerHTML = '<div class="loading">Error loading freelancers.</div>';
  }
}
...
async function loadMore() {
  ...
  btn.innerText = "Load More Talent";
  btn.disabled = false;
}
...
function renderFreelancerCards() {
  ...
  if (allFreelancers.length === 0) {
    grid.innerHTML = '<div class="loading">No freelancers found</div>';
    return;
  }
  ...
  allFreelancers.forEach(f => {
    ...
    div.innerHTML = `
       ...
       <div class="skill">${f.skills}</div>
       <div class="department" style="margin-bottom:8px;">${f.department}</div>
       <div class="price">Starting at PKR ${f.stipend || '0'}</div>
       <div class="rating">
       ...
       <div class="button-group">
         ...
         <button class="btn view-profile" onclick='openWorkGallery(${JSON.stringify(f.workMedia || []).replace(/"/g,"&quot;")})'>
           View Work
         </button>
       </div>
...
async function saveFreelancer(name, email, profilePic) {
  const token = localStorage.getItem("access_token");
  if (!token) { showToast("Please login to save freelancers.", "warning"); return; }
  try {
    ...
    if (saved.some(f => f.user_email === email)) { showToast("Freelancer already saved", "info"); return; }
    ...
    if (patchRes.ok) showToast("Freelancer saved!", "success");
  } catch (err) { showToast("Error saving freelancer", "error"); }
}
...
async function loadUserProfile() {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      document.getElementById("navUserName").innerText = data.name || "Client";
    }
  } catch (err) {
